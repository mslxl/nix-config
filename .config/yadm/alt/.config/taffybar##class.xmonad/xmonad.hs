...
myKeyMaps :: XConfig Layout -> M.Map (KeyMask, KeySym) (X ())
myKeyMaps conf@(XConfig {XMonad.modMask = modm}) =
  ...
  where
    viewWinVM = do
      ...
      return ()
    actXRestart = spawn "notify-send \"XMonad compiler\" \"Start compile xmonad.hs\" && xmonad --recompile && xmonad --restart && notify-send \"XMonad compiler\" \"XMonad restart successfully\""
    actXExit = (io $ exitWith ExitSuccess)
