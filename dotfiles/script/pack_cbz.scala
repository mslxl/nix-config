...
def hasSubFolder(dir:os.Path):Boolean = os.list(dir)
  .find(os.isDir)
  .map(_ => true)
  .getOrElse(false)

def hasFile(dir:os.Path):Boolean = os.list(dir)
  .find(os.isFile)
  .map(_ => true)
  .getOrElse(false)
...
def pack(storePath:os.Path)(dir:os.Path):Unit = {
  ...
  val files = os.list(dir)
    .filter(path => os.isFile(path) && Array("jpg", "png", "jpeg").contains(path.ext))
